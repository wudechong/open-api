<template lang="pug">
	hp-modal-dialog(ref="modelDialog" :title="title" v-dialogDrag :width="width" :close-on-click-modal="false" @close="()=>close()")
					...
					.box
						el-form#formCondition(:inline="true")
							el-form-item(:label="$t('biz.goodsPicker.goodsName')")
								el-input(v-model="form.goods" placeholder="支持多关键字" @keyup.enter.native="keyupEnter")
							...
							el-form-item(:label="$t('biz.goodsPicker.productNo')" v-if="showProductNo")
								el-input(v-model="form.productNo" placeholder="货号" @keyup.enter.native="keyupEnter")
							el-form-item(label="条码：" v-if="showBarCode")
								el-input(v-model="form.barcodeSearcher" placeholder="条码" @keyup.enter.native="keyupEnter")
							...
							el-form-item(v-if="openInventoryZCP")
								el-checkbox(v-model="form.haveDefectInventory") 仅显示有次品库存的商品
							el-form-item(v-if="onlySupplier")
								el-checkbox(v-model="form.supplierOwner") 仅显示供应商可供商品
							el-form-item
						...
						.toolbar
							hp-pager(ref="pager" :showTotal="showTotal" :pageSize="pageSize" :pageNo.sync="pageNo" :pageCount.sync="pageCount" :totalCount="total"
								:loading="loading" isGoods :showPageSize="false" @pageChanged="pageChanged" @toggle="toggle")
					.flex-box
						vxe-table.table-goods(ref="tableGoods" width="100%" height="100%" :data="tableData" border="none"
							...
							vxe-table-column(field="productNo" title='货号' width="100")
							vxe-table-column(field="weight" title="重量" v-if="showOther")
							vxe-table-column(field="salePrice" :width="120" :title="$t('biz.goodsPicker.salePrice')" align="right" header-align="center")
								template(v-slot="{ row }")
									span {{ (row.salePrice) | toFixedN(getPricePrecision, currencySymbol) }}
							vxe-table-column(field="primePrice" title="参考进价" width="100" align="right" header-align="center" v-if="showOther")
								template(v-slot="{ row }")
									span {{ (row.primePrice) | toFixedN(getPricePrecision, currencySymbol)}}
							vxe-table-column(field="curPrice" title="成本价" width="100" align="right" header-align="center" v-if="showOther")
								template(v-slot="{ row }")
									span {{ (row.curPrice) | toFixedN(getPricePrecision, currencySymbol)}}
							...
							//- 	template(v-slot="{ row }")
							//- 		span {{ permission.csw_open_occupy_inventory ? row.occupy : row.available }}
							vxe-table-column(field="curAvaliable" title="当前仓库(可用)" v-if='(isShowQuantity || showOther) && !openInventoryZCP')
								template(v-slot="{ row }")
									span(v-if="row.goodsType != 1") {{row.curQuantity | toFixedN(getNumPrecision, '')}}
									span(v-if="row.curAvaliable || row.curAvaliable ==0") {{'('}}
									span(v-if="row.curAvaliable || row.curAvaliable ==0") {{row.curAvaliable | toFixedN(getNumPrecision, '')}}
									span(v-if="row.curAvaliable || row.curAvaliable ==0") {{')'}}
									span(v-if="!row.curAvaliable && !row.curAvaliable ==0") {{''}}
							vxe-table-colgroup(field="curQuantity" title="当前仓库(可用)" width="100" align="right" header-align="center" v-if='(isShowQuantity || showOther) && openInventoryZCP')
								vxe-table-column(field="curAvaliable" title="正品" width="100" align="center")
									template(v-slot="{ row }")
										span(v-if="row.goodsType != 1") {{row.curQuantity | toFixedN(getNumPrecision, '')}}
										span(v-if="row.curAvaliable || row.curAvaliable ==0") {{'('}}
										span(v-if="row.curAvaliable || row.curAvaliable ==0") {{row.curAvaliable | toFixedN(getNumPrecision, '')}}
										span(v-if="row.curAvaliable || row.curAvaliable ==0") {{')'}}
										span(v-if="!row.curAvaliable && !row.curAvaliable ==0") {{''}}
								vxe-table-column(field="curAvaliableDefect" title="次品" width="100" align="center")
									template(v-slot="{ row }")
										span(v-if="row.goodsType != 1") {{row.curQuantityDefect | toFixedN(getNumPrecision, '')}}
										span(v-if="row.curAvaliableDefect || row.curAvaliableDefect ==0") {{'('}}
										span(v-if="row.curAvaliableDefect || row.curAvaliableDefect ==0") {{row.curAvaliableDefect | toFixedN(getNumPrecision, '')}}
										span(v-if="row.curAvaliableDefect || row.curAvaliableDefect ==0") {{')'}}
										span(v-if="!row.curAvaliableDefect && !row.curAvaliableDefect ==0") {{''}}
...
export default {
	...
	data() {
		return {
			...
			showBarCode: false,
			onlySupplier: false,
			openInventoryZCP: false,  // 仅显示有次品库存的商品
	...
	computed: {
		...mapState("common", ["brand"]),
		...mapState("permission", ["permission"]),
		...mapState('configCommon',['getPricePrecision', 'getNumPrecision']),
	},
	created() {
	...
	methods: {
		getCurrencyCode () {
			this.$http.get("/calf/basic/widget/currency/queryDefault").then(res => {
				this.currencySymbol = res?.symbol ? res.symbol: '¥'
			})
		},
		open(options) {
			...
			this.showOther = options.showOther ?? false;
			this.openInventoryZCP = options.openInventoryZCP ?? false;
			this.isShowQuantity = options.isShowQuantity ?? false;
			this.onlySupplier = options.onlySupplier ?? false;
			this.isShowNumColumn = options.isShowNumColumn ?? false;
			this.clickChoose = options.clickChoose ?? false;
			this.goodsLimit = options.goodsLimit ?? 0;
			this.showGroupShop = options.showGroupShop ?? true;
			this.form.supplierID = options.supplierID ?? undefined;
			this.form.warehouseID = options.warehouseID ?? undefined;
			this.isLoad = false;
			this.showTree = true;
			this.initValue();
			//可移动的弹窗去除移动距离
		...
		async loadNode(node, resolve) {
			...
			if (this.isGroup) {
				...
				if (node.level === 1) {
					if (this.isLoad) return resolve([]);
					let twoData = this.showGroupShop ? [
						{
							categoryID: "1",
			...
			try {
				const res = await this.$http.post("/service/erp/goods/select", {
					...
					goodsType: this.isGroup
						...
						: "",
					keyWordType: this.form.keyword ? 'nameAndCode' : undefined,
					supplierOwner: this.form.supplierOwner ? true : undefined,
					pageNo: this.pageNo,
