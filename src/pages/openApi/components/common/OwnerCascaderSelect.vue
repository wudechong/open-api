				...
				.content
					el-cascader-panel(
						...
						template(slot-scope="{node, data}")
							span(v-if="data.extra") {{data[realProps.label]}} {{data.extra}}
					...
					ul.suggestions(v-if="filterValue" :class="{multiple:multiple}")
						li.suggestion(v-for="(suggestion, num) in suggestions" :key="num" :class="{selected: !multiple && isSuggestionSelected(suggestion), 'focus': focusNum===num}"
							...
							span.label(@click="clickLabel(suggestion, num, $event)") {{suggestion.label}}
								span(v-if="showDefLabel") {{suggestion.extra}}
				.footer(v-if="addOperate && !filterValue")
					el-button.button-w.x-center(icon="el-icon-plus" @click="$emit('addOperate')") 新增供应商
...
export default {
	...
	methods: {
		...
		async setValue(value) {
			this.$refs.cascader && this.$refs.cascader.syncMultiCheckState()
			setTimeout(() => {
				this.$forceUpdate();
			});
		...
		async dropdownVisibleChange(visible) {
			...
			const inputFilter = this.$refs && this.$refs.inputFilter && this.$refs.inputFilter.$el
			if (inputFilter) {
				if (visible) {
					this.$nextTick(() => {
